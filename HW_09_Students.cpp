...
/*
Student binarySearch(const std::vector<Student>& vec, const std::string str) {
...
if (std::get<0>(vec[middle]) == str) {
return vec[middle];
}
else if (std::get<0>(vec[middle]) < str) {
left = ++middle;
}
else {
right = --middle;
...
void sortName(const Group& gr) {
auto& students = std::get<1>(gr);
...
for (auto a : students) {
vecNames.push_back(std::get<0>(a));
}
